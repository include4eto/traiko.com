...
class Encoder:
    ...
    def save(self):
        ...
        """
        config = json.loads(self.model.model_config)

        for layer in config['config']:
            if 'dilation_rate' in layer['config']:
                layer['config']['dilation_rate'] = \
                    layer['config']['dilation_rate'][0]
                layer['config']['strides'] = \
                    layer['config']['strides'][0]
                
                print(layer['config']['dilation_rate'])
                print(layer['config']['strides'])
                print(layer['config']['kernel_size'])
                
        self.model.model_config = json.dumps(config)
